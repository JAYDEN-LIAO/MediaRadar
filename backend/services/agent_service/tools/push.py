...
P1 限制：
- RSS 通道暂未实现且被显式拒绝，因为 RSS 是 pull 模式（用户主动订阅
  /rss/{token}.xml 获取），与 email/wecom/feishu 的 push 模式（系统主动
  推送）本质不同。Agent 只管理 push 通道；RSS 由 api.py:/rss/{token}.xml
  直接生成 XML，无需 Agent 介入。
...
def toggle_channel_tool(channel: str, enabled: bool, _owner_id: str) -> str:
    if channel not in _SUPPORTED_CHANNELS:
        return ToolResult(
            success=False,
            error=f"不支持的通道: {channel}（push 模式仅支持 email/wecom/feishu；RSS 为 pull 模式，需直接订阅 /rss/ 端点）",
    ...
    reload_registry(owner_id=_owner_id)  # v2.2: per-user 热重载
...
def update_channel_config_tool(channel: str, config: dict, _owner_id: str) -> str:
    ...
    reload_registry(owner_id=_owner_id)
